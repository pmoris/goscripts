...
def multiproc_enrichmentAnalysis(GOdict, gafDict, gafSubset,
                       minGenes=3, threshold=0.05, proc=mp.cpu_count() - 1):
    """
    Performs a GO enrichment analysis.
    ...
    subsetGOidsParents = {parent for GOid in subsetGOids for parent in GOdict[GOid].recursive_parents}
    baseGOids = [GOid for GOid in subsetGOids if GOid not in subsetGOidsParents]
    print('NUMBER OF baseGOids', len(baseGOids))
    ...
    t = time.clock()
    results1 = Parallel(n_jobs=-2, backend='threading', verbose=10, batch_size=1)(delayed(recursive_wrapper)( GOid, backgroundTotal,
                                                 subsetTotal, GOdict,
                                                 gafDict, gafSubset,
                                                 minGenes, threshold )
                                                 for GOid in baseGOids)
    thread_time = time.clock() - t

    t = time.clock()
    results2 = Parallel(n_jobs=-2, backend='multiprocessing', verbose=10)(delayed(recursive_wrapper)( GOid, backgroundTotal,
                                                 subsetTotal, GOdict,
                                                 gafDict, gafSubset,
                                                 minGenes, threshold )
                                                 for GOid in baseGOids)
    multiproc_time = time.clock() - t

    t = time.clock()
    results3 = Parallel(n_jobs=-2, backend='multiprocessing', verbose=10, batch_size=700)(delayed(recursive_wrapper)( GOid, backgroundTotal,
                                                 subsetTotal, GOdict,
                                                 gafDict, gafSubset,
                                                 minGenes, threshold )
                                                 for GOid in baseGOids)
    multiproc_700_time = time.clock() - t

    print('thread:',thread_time)
    print('multi_auto:',multiproc_time)
    print('multi_700:',multiproc_700_time)

    print('results are equal?', results1 == results2)
    print('results are equal?', results3 == results2)
...
def recursiveTester(GOid, backgroundTotal, subsetTotal, GOdict, gafDict,
    ...
    if GOid not in enrichmentTestResults['pValues']:
        ...
        validTerms = {GOid}  # https://stackoverflow.com/questions/36674083/why-is-it-possible-to-replace-set-with
